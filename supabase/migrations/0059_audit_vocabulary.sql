-- ═══════════════════════════════════════════════════════════════════════════════
-- 0059 · the vocabulary intake speaks
-- ═══════════════════════════════════════════════════════════════════════════════
--
...
--
-- THE LIST IS GENERATED FROM THE UNION, IN ORDER, NOT TYPED. 156 actions.
-- ═══════════════════════════════════════════════════════════════════════════════
...
alter table public.audit_events add constraint audit_events_action_check
  check (action in (
    ...
    'CLIENT_UPDATED',
    'MATTER_CREATED',
    'MATTER_REPORT_UPDATED',
    'MATTER_TEAM_ASSIGNED',
    'MATTER_TEAM_UNASSIGNED',
    'PARTY_CREATED',
    'PARTY_UPDATED',
    'PARTY_MERGED',
    'PARTY_ALIAS_ADDED',
...
begin
  if not exists (
    ...
    raise exception '0059: audit_events_action_check was not created';
  end if;
  ...
  if admitted = 0 then
    raise exception '0059: the constraint does not look like an action list';
  end if;
  ...
  if missing is not null then
    raise exception '0059: declared but not admitted: %', missing;
  end if;

  raise notice '0059: audit vocabulary at 156 actions — verified against the declared union';
end $$;
